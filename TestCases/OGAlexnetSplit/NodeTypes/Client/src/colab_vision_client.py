...
from . import alexnet_pytorch_split as alex
...
client_mode = "cpu"
test_results_dir = pathlib.Path(sys.path[0]).parent.parent / "Output"
...
class FileClient:
    def __init__(self, address, result_output=test_results_dir):
        ...
        self.model = alex.Model(
            mode=client_mode,
            imgnet_classes_fp=Path(parent).parent.parent
            / "Datasets"
            / "test"
    ...
    def safeClose(self):
        ...
        df.to_csv(self.result_output / f"test_results__{current_datetime}.csv")
