...
class CloudService(ParticipantService):
    ...
    def exposed_complete_inference(self, serialized_tensor, parent_uuid, start_layer):
        ...
        threading.Thread(
            target=complete_inference,
            args=[x],
            kwargs={"start": start_layer, "inference_id": parent_uuid + ".0"},
        ).start()
...
if __name__ == "__main__":
    ...
    this_server = ThreadedServer(
        CloudService(), auto_register=True, protocol_config={"allow_public_attrs": True}
    )
